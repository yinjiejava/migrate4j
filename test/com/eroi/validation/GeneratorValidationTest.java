...
import com.eroi.migrate.generators.OracleGenerator;
...
public class GeneratorValidationTest extends TestCase {
	...
	public void testAddColumnAfterColumn_Version5To6() throws Exception {
		...
		// Postgres, Derby and Oracle do not support adding columns using BEFORE or AFTER so let's skip
		// this test.
		...
		if (genClass != PostgreSQLGenerator.class &&
			...
			genClass != OracleGenerator.class) {
	...
	private void insertDescIntoBasicTable() throws SQLException {
		...
		boolean autoIncrementColumnsSupported = true;
		if (generator instanceof OracleGenerator) autoIncrementColumnsSupported = false;
		String query = null;

		if (autoIncrementColumnsSupported) {
			query = "insert into "
